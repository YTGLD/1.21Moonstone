...
public class diemug  {
    ...
    public diemug() {
        super(new Properties().stacksTo(1).component(DataComponents.BUNDLE_CONTENTS,BundleContents.EMPTY).rarity(Rarity.EPIC).food(
    ...
    public ItemStack finishUsingItem(ItemStack stack, Level level, LivingEntity living){
        ...
        if (bundlecontents != null && !bundlecontents.isEmpty()){
            bundlecontents.itemsCopy().forEach((itemStack)->{
                List<MobEffectInstance> mobEffect = PotionIt
            });
        }
