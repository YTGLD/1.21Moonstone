...
public class BlackCube {
    ...
    public void renderCircle(PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, float x, float y, Entity entity) {
        ...
        for (int i = 0; i < segments; i++) {
            ...
            vertexConsumer.addVertex(poseStack.last().pose(), x1, y1, 0).setColor(0, 0, 0, 1.0f).setUv(0, 0).setOverlay(OverlayTexture.NO_OVERLAY).setUv2(packedLight,packedLight).setNormal(1, 0, 0);
            vertexConsumer.addVertex(poseStack.last().pose(), x2, y2, 0).setColor(0, 0, 0, 1.0f).setUv(0, 0).setOverlay(OverlayTexture.NO_OVERLAY).setUv2(packedLight,packedLight).setNormal(1, 0, 0);
        }
    }
